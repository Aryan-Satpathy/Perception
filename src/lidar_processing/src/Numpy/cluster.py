...
class DbCluster:
    def __init__(self, epsilon = 0.3, min_points = 50, print_progress = False, log_time = True, debug = False) -> DbCluster:
        self.eps = epsilon
    ...
    def __call__(self, pcl: open3d.geometry.PointCloud) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Copy the pointcloud so that 2D projection wont affect the original cloud
        pcl = copy.copy(pcl)

        # 2D
        pts = np.asarray(pcl.points)
        pts[:, 2] = np.zeros(pts[:, 2].shape)
        pcl.points = open3d.utility.Vector3dVector(pts)
