...
class ROI:
    ...
    def __call__(self, points_np: np.array) -> np.array:
        indexes_ubound = np.where(np.all(points_np <= [self.X_MAX, self.Y_MAX, self.Z_MAX,  np.inf], axis = -1))
        indexes_lbound = np.where(np.all(points_np >= [self.X_MIN, self.Y_MIN, self.Z_MIN, -np.inf], axis = -1))
...
class Downsample:
    def __init__(self, vox_size_dense: float = 0.05, vox_size_sparse: float = 0.01, x_lim: float = 2) -> None:
