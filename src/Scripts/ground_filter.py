...
def callback_pointcloud(raw_cloud):
    ...
    z_axis_filtered_indices = np.logical_and(lidar_points[:,2] < 2.7,lidar_points[:,2] > 0)
    ...
    ground_filtered_cloud = ros_numpy.point_cloud2.array_to_pointcloud2(for_ground, stamp = raw_cloud.header.stamp, frame_id='rslidar_front')
...
def filter_ground():
    ...
    rospy.Subscriber("/rslidar_points_front", PointCloud2, callback_pointcloud)
